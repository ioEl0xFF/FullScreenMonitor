    ...
    public partial class SettingsWindow : Window, INotifyPropertyChanged
    {
        ...
        private void Save_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("監視対象プロセスを1つ以上設定してください。", "設定エラー", 
                        MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                {
                    MessageBox.Show("監視間隔は100ms〜2000msの範囲で設定してください。", "設定エラー", 
                        MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                if (StartWithWindows != originalStartWithWindows)
                {
                    if (StartWithWindows)
                    {
                        ...
                        {
                            MessageBox.Show("スタートアップ登録に失敗しました。", "エラー", 
                    ...
                    else
                    {
                        ...
                        {
                            MessageBox.Show("スタートアップ解除に失敗しました。", "エラー", 
                                MessageBoxButton.OK, MessageBoxImage.Error);
                            return;
                        }
                    }
            ...
            {
                MessageBox.Show($"設定保存中にエラーが発生しました:\n{ex.Message}", "エラー", 
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
